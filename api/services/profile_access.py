"""Profile-scoped access control for /api/helix management routes."""
...
def resolve_admin_helix_profile() -> str:
    ...
    return (os.getenv("HELIX_TELEGRAM_ADMIN_PROFILE") or DEFAULT_ADMIN_PROFILE).strip() or DEFAULT_ADMIN_PROFILE
...
def verify_profile_management(
    ...
    *,
    ...
    - ``X-Helix-Profile-Key`` matches the admin profile master key, or
    - header profile matches target and key unlocks that profile.
    """
    target = target_profile.strip()
    header_profile = (x_helix_profile or "").strip()
    ...
    key = (x_helix_profile_key or "").strip()
    if not key:
        raise HTTPException(
            status_code=401,
            detail="X-Helix-Profile-Key required for profile management",
        )

    admin_profile = resolve_admin_helix_profile()
    ...
    if header_profile and header_profile != target:
        raise HTTPException(
            status_code=403,
            detail="X-Helix-Profile must match the target profile for non-admin access",
        )
