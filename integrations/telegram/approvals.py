...
class TelegramApprovals:
    ...
    async def on_confirmation_request(self, event: ConfirmationRequestEvent) -> None:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="✓ Раз", callback_data=f"cfm:{event.confirmation_id}:1"),
                    InlineKeyboardButton(text="✓ Сессия", callback_data=f"cfm:{event.confirmation_id}:2"),
                ],
                [
                    InlineKeyboardButton(text="✓ Всегда", callback_data=f"cfm:{event.confirmation_id}:3"),
                    InlineKeyboardButton(text="✗ Отмена", callback_data=f"cfm:{event.confirmation_id}:4"),
                ],
            ]
        )
