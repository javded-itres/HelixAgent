...
| Душа агента | `~/.helix/profiles/<имя>/SOUL.md` |
| Профиль пользователя | `~/.helix/profiles/<имя>/USER.md` |
| Маркер первого запуска | `~/.helix/profiles/<имя>/INIT.md` (удаляется после онбординга) |
...
| `INIT.md` | Маркер первого запуска — пока файл есть, Helix проводит короткий онбординг |

При `helix profile create <имя>` Helix создаёт `INIT.md` и заготовку `SOUL.md`.
...
```bash
helix -p alice profile env --edit   # только секреты
# файлы идентичности:
