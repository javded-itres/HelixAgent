...
|-------------|-----------|
| Нет автопривязки `user_id → профиль` | Пользователь сам выбирает `/profile` |
...
Автоматическая привязка «Telegram user id → профиль» в одном боте **пока не реализована** в Helix.