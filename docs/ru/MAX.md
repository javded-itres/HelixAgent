...
Один бот MAX обслуживает несколько профилей Holix. Учётные данные хранятся в `profiles/{profile}/max.env` (шифрование при включённом profile crypto).

| Сценарий | Описание |
|----------|----------|
| **Запросы доступа** | Новые пользователи шлют `/start`; админ одобряет через `helix max requests` |
| **Привязки** | `user_id → holix_profile` через `helix max map` |
| **Админ** | Один MAX-админ (`helix max admin` или `--set-admin`) — `/message`, `/init`, установка MCP |
| **Ключ профиля** | Одобренные пользователи разблокируют профиль access key |
...
После смены env или привязок при работающем gateway: `holix gateway reload` (переподписка MAX webhook для host-профиля).

## Команды CLI

| Команда | Описание |
|---------|----------|
| `helix max setup` | Мастер: токен, allowlist, режим, `profiles/{p}/max.env` |
| `helix max` | Запуск бота (Long Polling — только dev/test) |
| `helix max status` | Токен, админ, карта пользователей, ожидающие заявки, подписки |
| `helix max map` | Привязки user → profile |
| `helix max requests` | Список/одобрение/отклонение заявок |
| `helix max admin` | Показать/сбросить администратора MAX |
| `helix gateway start` | Gateway + MAX webhook companion |
| `helix gateway status` | Health gateway + сводка MAX (env/admin/map) |
...
| Пользователь игнорируется | Одобрите через `helix max requests approve` или добавьте в allowlist |
