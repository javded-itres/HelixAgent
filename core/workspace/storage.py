"""Workspace file I/O with optional encryption and quota enforcement."""

from __future__ import annotations

from pathlib import Path

from core.crypto.encrypted_fs import is_encrypted_file, read_encrypted_text, write_encrypted_text
from core.crypto.profile_crypto import is_profile_encryption_enabled
from core.crypto.unlock_context import require_profile_dek
...
def read_profile_file_text(file_path: Path, *, profile: str, encoding: str = "utf-8") -> str:
    if should_encrypt_path(file_path, profile):
        dek = require_profile_dek(profile)
...
def write_profile_file_text(
    ...
    *,
    ...
    """Write file; return bytes stored on disk."""
    ...
    if should_encrypt_path(file_path, profile):
        dek = require_profile_dek(profile)
        projected = len(content.encode(encoding)) + 9 + 12 + 16
        if root is not None:
            check_workspace_write(
                profile=profile,
                workspace_root=root,
                target=file_path,
                new_payload_bytes=projected,
                previous_size=old_size,
            )
        new_size = write_encrypted_text(file_path, dek, content, encoding=encoding)
    else:
